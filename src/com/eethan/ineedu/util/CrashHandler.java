...
public class CrashHandler implements UncaughtExceptionHandler {
	...
	private boolean handleException(Throwable ex) {
		...
		new Thread() {
			...
			public void run() {
				Looper.prepare();
				MyToast.showToast(mContext, "很抱歉,程序出现异常,即将退出...");
				Looper.loop();
			}
		}.start();
		//收集设备参数信息 
	...
	public void uploadCrashFile(final File file) {
    	new Thread(new Runnable() {
			...
			public void run() {
				// TODO Auto-generated method stub
				String requestUrl = URLConstant.UPLOAD_CRASH_FILE_URL;
				// 请求普通信息
				...
				Log.i(TAG, "upload CrashFile success");
			}
		}).start();

...
import com.eethan.ineedu.adapter.PhotoNewsDetailsAdapter.CommentItem;
...
public class MoodDetailAdapter extends BaseAdapter {
	...
	private List<MoodDetailModel> commList = new ArrayList<MoodDetailModel>();
	private ImageLoader imageLoader = ImageLoader.getInstance();
	private DisplayImageOptions options_head;
	private int ownerid;

	public MoodDetailAdapter(Context cnt, List<MoodDetailModel> list,
			int ownerId) {
		context = cnt;
		commList = list;
	...
	public View getView(int position, View view, ViewGroup parent) {
		...
		String readableTime = DataTraslator.LongToTimePastGeneral(Long
				.parseLong(commList.get(pos).getMoodComment().getTime()));
		...
		String distanceString = DataTraslator
				.DistanceToString(DataTraslator.GetDistanceToMe(context,
						commList.get(pos).getUserLocation().getLat(), commList
								.get(pos).getUserLocation().getLng()));
		...
		int tarId = commList.get(pos).getMoodComment().getCommentedUserId();
		if (tarId >0) {
			...
			for (int i = 0; i < num; i++) {
				if (commList.get(i).getUserInfo().getUserId() == tarId) {
					...
					item.contentTxtVw.setText("回复 " + namesString + " :  "
							+ commList.get(pos).getMoodComment().getContent());
					break;
				}
			}
		} else {
			item.contentTxtVw.setText(commList.get(pos).getMoodComment()
					.getContent());
		}

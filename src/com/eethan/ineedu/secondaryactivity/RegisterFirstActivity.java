	...
	private Handler mHandler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			if (result.equals("emailRepetition")) {
				email.setError(Html
						.fromHtml("<font color=#E10979>邮箱已被使用</font>"));
				focusView = email;
				focusView.requestFocus();
				return;
			} else if (result.equals("phoneRepetition")) {
				phone.setError(Html
						.fromHtml("<font color=#E10979>号码已被使用</font>"));
				focusView = phone;
				focusView.requestFocus();
				return;
		...
		nextButton.setOnClickListener(new OnClickListener() {
					...
					new Thread(new Runnable() {
						...
						public void run() {
							...
							try {
								...
								String checkresult = ServerCommunication
										.request(checkRigisteruser,
												URLConstant.CHECK_RIGISTER_URL);

								result = JacksonUtil.json().fromJsonToObject(
										checkresult, String.class);
			...
			private boolean checkInfo() {
				...
				if (!InfoCheckUtil.isPhoneNum(phoneString)) {
					phone.setError(Html
							.fromHtml("<font color=#E10979>请输入正确的手机号码</font>"));
					focusView = phone;
					return false;
				}
				if (!InfoCheckUtil.isEmail(emailString)) {
					email.setError(Html
							.fromHtml("<font color=#E10979>请输入正确的邮箱</font>"));
					focusView = email;
					return false;
				}
				if (pwdString.isEmpty()) {
					pwd.setError(Html
							.fromHtml("<font color=#E10979>请输入密码</font>"));
					focusView = pwd;
					return false;
				}
				...
				if (pwdString.length() > 16) {
					pwd.setError(Html
							.fromHtml("<font color=#E10979>密码长度不超过16位</font>"));
					focusView = pwd;
					return false;
				}

				if (!pwdString.equals(pwd_againString)) {
					pwd_again.setError(Html
							.fromHtml("<font color=#E10979>两次输入的密码不同</font>"));
					focusView = pwd_again;
					return false;
				}
				return true;
